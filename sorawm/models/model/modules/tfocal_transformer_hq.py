...
class SoftSplit(nn.Module):
    ...
    def forward(self, x, b, output_size):
        f_h = int(
            (
                output_size[0]
                + 2 * self.t2t_param["padding"][0]
                - (self.t2t_param["kernel_size"][0] - 1)
                - 1
            )
            / self.t2t_param["stride"][0]
            + 1
        )
        f_w = int(
            (
                output_size[1]
                + 2 * self.t2t_param["padding"][1]
                - (self.t2t_param["kernel_size"][1] - 1)
                - 1
            )
            / self.t2t_param["stride"][1]
            + 1
        )
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if any(i > 0 for i in self.expand_size) and focal_level > 0:
            ...
            mask_rolled = torch.stack((mask_tl, mask_tr, mask_bl, mask_br), 0).flatten(
                0
            )
            self.register_buffer(
                "valid_ind_rolled", mask_rolled.nonzero(as_tuple=False).view(-1)
            )
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level - 1):
                stride = 2**k
                kernel_size = tuple(
                    2 * (i // 2) + 2**k + (2**k - 1) for i in self.focal_window
                )
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if any(i > 0 for i in self.expand_size) and self.focal_level > 0:
            (k_tl, v_tl) = map(
                lambda t: torch.roll(
                    t, shifts=(-self.expand_size[0], -self.expand_size[1]), dims=(2, 3)
                ),
                (k, v),
            )
            (k_tr, v_tr) = map(
            ...
            k_rolled = (
                torch.cat((k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows), 2)
                .permute(0, 3, 1, 2, 4)
                .contiguous()
            )
            v_rolled = (
                torch.cat((v_tl_windows, v_tr_windows, v_bl_windows, v_br_windows), 2)
                .permute(0, 3, 1, 2, 4)
                .contiguous()
            )
            ...
            k_rolled = k_rolled.view(
                -1, self.num_heads, T * temp_N, C // self.num_heads
            )
            v_rolled = v_rolled.view(
                -1, self.num_heads, T * temp_N, C // self.num_heads
            )
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                stride = 2**k
                # B, T, nWh, nWw, C
                x_window_pooled = x_all[k + 1].permute(0, 3, 1, 2, 4).contiguous()
                ...
                unfolded_mask = (
                    self.unfolds[k](mask.unsqueeze(1))
                    .view(
                        1,
                        T,
                ...
                x_window_masks = x_window_masks.masked_fill(
                    x_window_masks == 0, float(-100.0)
                ).masked_fill(x_window_masks > 0, float(0.0))
                mask_all[k + 1] = x_window_masks

                # generate k and v for pooled windows
                qkv_pooled = (
                    self.qkv(x_window_pooled)
                    .reshape(B, T, nWh, nWw, 3, C)
                    .permute(4, 0, 1, 5, 2, 3)
                    .view(3, -1, C, nWh, nWw)
                    .contiguous()
                )
                ...
                k_pooled_k = k_pooled_k.view(
                    -1,
                    self.num_heads,
                    T * self.unfolds[k].kernel_size[0] * self.unfolds[k].kernel_size[1],
                    C // self.num_heads,
                )
        ...
        if mask_all[0] is not None:
            ...
            attn[:, :, :, :, :window_area] = (
                attn[:, :, :, :, :window_area] + mask_all[0][None, :, None, :, :]
            )
...
class TemporalFocalTransformerBlock(nn.Module):
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level - 1):
                ...
                self.pool_layers[-1].weight.data.fill_(
                    1.0 / (window_size_glo[0] * window_size_glo[1])
                )
    ...
    def forward(self, x):
        ...
        shifted_x = window_reverse(
            attn_windows, self.window_size, T, H, W
        )  # B T H' W' C

        # FFN
        x = shortcut + shifted_x
        y = self.norm2(x)
        x = x + self.mlp(y.view(B, T * H * W, C), output_size).view(B, T, H, W, C)
