...
class SoftSplit(nn.Module):
    def __init__(self, channel, hidden, kernel_size, stride, padding, t2t_param):
        ...
        self.f_h = int(
            (
                t2t_param["output_size"][0]
                + 2 * t2t_param["padding"][0]
                - (t2t_param["kernel_size"][0] - 1)
                - 1
            )
            / t2t_param["stride"][0]
            + 1
        ...
        self.f_w = int(
            (
                t2t_param["output_size"][1]
                + 2 * t2t_param["padding"][1]
                - (t2t_param["kernel_size"][1] - 1)
                - 1
            )
            / t2t_param["stride"][1]
            + 1
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                stride = 2**k
                x_window_pooled = (
                    x_all[k + 1].permute(0, 3, 1, 2, 4).contiguous()
                )  # B, T, nWh, nWw, C
...
class TemporalFocalTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_windows = self.attn(
            x_windows_all, mask_all=x_window_masks_all
